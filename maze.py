...
class Maze(object):
    ...
    def sensor_model(self, coordinates, sensor_limit, orientation = 0):
        ...
        '''
        Return:
        (up right, rear right, rear left, up left)
        '''
        pos_x = x
        pos_y = y
        d5 = 0
        ...
        pos_y = y
        d7 = 0
        dx = np.cos(orientation-np.pi*5/4) * 1 - np.sin(orientation-np.pi*5/4) * 0
        dy = np.sin(orientation-np.pi*5/4) * 1 + np.cos(orientation-np.pi*5/4) * 0
        ...
        pos_y = y
        d8 = 0
        dx = np.cos(orientation-np.pi*7/4) * 1 - np.sin(orientation-np.pi*7/4) * 0
        dy = np.sin(orientation-np.pi*7/4) * 1 + np.cos(orientation-np.pi*7/4) * 0
